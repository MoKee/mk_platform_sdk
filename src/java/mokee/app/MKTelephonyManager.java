 ...
 *
 * <p>
 * To get the instance of this class, utilize MKTelephonyManager#getInstance(Context context)
 */
public class MKTelephonyManager {
    ...
    private static final String TAG = "MKTelephonyManager";
    private static boolean localLOGD = Log.isLoggable(TAG, Log.DEBUG);

    private static IMKTelephonyManager sService;
    private static MKTelephonyManager sMKTelephonyManagerInstance;
    private Context mContext;

    private MKTelephonyManager(Context context) {
    ...
    /**
     * Get or create an instance of the {@link mokee.app.MKTelephonyManager}
     *
     * @return {@link mokee.app.MKTelephonyManager}
     */
    public static MKTelephonyManager getInstance(Context context) {
        if (sMKTelephonyManagerInstance == null) {
            sMKTelephonyManagerInstance = new MKTelephonyManager(context);
        }
        return sMKTelephonyManagerInstance;
    }

    /** @hide */
    public IMKTelephonyManager getService() {
        if (sService != null) {
            return sService;
        }
        IBinder b = ServiceManager.getService(MKContextConstants.MK_TELEPHONY_MANAGER_SERVICE);
        if (b != null) {
            sService = IMKTelephonyManager.Stub.asInterface(b);
            return sService;
        }
        return null;
    }
    ...
    public List<SubscriptionInfo> getSubInformation() {
        if (sService == null) {
            Log.w(TAG, "not connected to MKTelephonyManager");
            return null;
        }
        ...
        try {
            ...
            Slog.w(TAG, "warning: no mk telephony manager service");
        }
