    ...
    public void onBootPhase(int phase) {
        if (phase == PHASE_SYSTEM_SERVICES_READY) {
            synchronized (this) {
                ...
                if (mNumProfiles > 0) {
                    int profile = MKSettings.Secure.getInt(mContext.getContentResolver(),
    ...
    private boolean hasAppProfiles() {
        return mNumProfiles > 0 && mPatterns != null &&
               (MKSettings.Secure.getInt(mContext.getContentResolver(),
                       MKSettings.Secure.APP_PERFORMANCE_PROFILES_ENABLED, 1) == 1);
    }
    ...
    private synchronized boolean setPowerProfileInternal(int profile, boolean fromUser) {
        ...
        if (fromUser) {
            MKSettings.Secure.putInt(mContext.getContentResolver(),
                    MKSettings.Secure.PERFORMANCE_PROFILE, profile);
        }
