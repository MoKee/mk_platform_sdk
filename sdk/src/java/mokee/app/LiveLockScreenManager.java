...
public class LiveLockScreenManager {
    ...
    public static final String SERVICE_INTERFACE
            = "mokee.app.LiveLockScreenManagerService";

    private LiveLockScreenManager(Context context) {
        mContext = context;
        sService = getService();
        if (context.getPackageManager().hasSystemFeature(
                MKContextConstants.Features.LIVE_LOCK_SCREEN) && sService == null) {
    ...
    private ILiveLockScreenManager getService() {
        if (sService == null) {
            IBinder b = ServiceManager.getService(MKContextConstants.MK_LIVE_LOCK_SCREEN_SERVICE);
            if (b != null) {
