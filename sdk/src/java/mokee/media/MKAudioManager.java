 ...
 * @hide
 */
public final class MKAudioManager {

    private static final String TAG = "MKAudioManager";
    ...
    private static MKAudioManager sInstance;
    private static IMKAudioService sService;

    /**
     * @hide to prevent subclassing from outside of the framework
     */
    private MKAudioManager(Context context) {
        ...
        if (!context.getPackageManager().hasSystemFeature(
                MKContextConstants.Features.AUDIO) || !checkService()) {
            throw new RuntimeException("Unable to get MKAudioService. The service either" +
    ...
    /**
     * Get or create an instance of the {@link mokee.media.MKAudioManager}
     * @param context
     * @return {@link MKAudioManager}
     */
    public synchronized static MKAudioManager getInstance(Context context) {
        if (sInstance == null) {
            sInstance = new MKAudioManager(context);
        }
        return sInstance;
    }

    /** @hide */
    public static IMKAudioService getService() {
        if (sService != null) {
            return sService;
        }
        IBinder b = ServiceManager.getService(MKContextConstants.MK_AUDIO_SERVICE);
        if (b != null) {
            sService = IMKAudioService.Stub.asInterface(b);
            return sService;
        }
        return null;
    }
    ...
    private boolean checkService() {
        if (sService == null) {
            Log.w(TAG, "not connected to MKAudioService");
            return false;
        }
        return true;
    }
